...
4. **Recruiting**: update statuses on the board (status changes with a week set become timeline events automatically — your storyline beats). Competing schools autocomplete the same way as the team picker, and archetype options narrow to whatever's valid for the position you picked. When editing an existing recruit, an **Abilities** section appears below the form — physical abilities are the fixed set that comes with their archetype (just assign each a tier), mental abilities are any of the shared pool, capped at 3. Every player on your own roster has the same Abilities editor on their detail page — Bronze/Silver/Gold/Platinum tiers, shown as a colored dot next to the ability name.
